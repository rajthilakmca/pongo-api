    ...
    describe('creating instances', () => {
        it('should be an instance of Money when created from tokens', (done) => {
            Money.fromTokens('10', fakeWAVES).then((money) => {
                getChai_1.expect(Money.isMoney(money)).to.be.true;
            }).then(() => done());
        });
        ...
        it('should create Money with Asset as the `asset` argument', (done) => {
            Promise.all([
                ...
                getChai_1.expect(Money.isMoney(moneys[0])).to.be.true;
                getChai_1.expect(Money.isMoney(moneys[1])).to.be.true;
            }).then(() => done());
        });
    ...
    describe('arithmetic operations', () => {
        it('should add Money with the same Asset', (done) => {
            Promise.all([
                ...
                getChai_1.expect(Money.isMoney(result)).to.be.true;
                getChai_1.expect(result.toTokens()).to.equal('3.00000000');
                const result2 = moneys[0].plus(moneys[1]);
                getChai_1.expect(Money.isMoney(result2)).to.be.true;
                getChai_1.expect(result2.toTokens()).to.equal('3.00000000');
            }).then(() => done());
        });
        it('should sub Money with the same Asset', (done) => {
            Promise.all([
                ...
                getChai_1.expect(Money.isMoney(result)).to.be.true;
                getChai_1.expect(result.toTokens()).to.equal('1.90000000');
                const result2 = moneys[0].minus(moneys[1]);
                getChai_1.expect(Money.isMoney(result2)).to.be.true;
                getChai_1.expect(result2.toTokens()).to.equal('1.90000000');
            }).then(() => done());
        });
        it('should throw when Money instances have different Asset instances', (done) => {
            Promise.all([
                ...
                getChai_1.expect(() => moneyOne.add(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.plus(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.sub(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.minus(moneyTwo)).to.throw();
            }).then(() => done());
        });
    });
    describe('arithmetic comparisons', () => {
        it('should properly compare instances of money', (done) => {
            Promise.all([
                ...
                getChai_1.expect(money2.eq(money3)).to.be.true;
                getChai_1.expect(money2.eq(money4)).to.be.false;
                getChai_1.expect(money1.lt(money2)).to.be.true;
                getChai_1.expect(money2.lt(money3)).to.be.false;
                getChai_1.expect(money4.lt(money3)).to.be.false;
                getChai_1.expect(money1.lte(money2)).to.be.true;
                getChai_1.expect(money2.lte(money3)).to.be.true;
                getChai_1.expect(money4.lte(money3)).to.be.false;
                getChai_1.expect(money4.gt(money3)).to.be.true;
                getChai_1.expect(money3.gt(money2)).to.be.false;
                getChai_1.expect(money3.gt(money4)).to.be.false;
                getChai_1.expect(money4.gte(money3)).to.be.true;
                getChai_1.expect(money3.gte(money2)).to.be.true;
                getChai_1.expect(money3.gte(money4)).to.be.false;
            }).then(() => done());
        });
        it('should throw when Money instances have different Asset instances', (done) => {
            Promise.all([
                ...
                getChai_1.expect(() => moneyOne.eq(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.lt(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.lte(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.gt(moneyTwo)).to.throw();
                getChai_1.expect(() => moneyOne.gte(moneyTwo)).to.throw();
            }).then(() => done());
        });
    });
    describe('conversions', () => {
        it('should clone Money with the same asset but different amount (from coins)', (done) => {
            Money.fromTokens('100', fakeWAVES).then((money) => {
                ...
                getChai_1.expect(emptyClone.toCoins()).to.equal('0');
                getChai_1.expect(emptyClone.toTokens()).to.equal('0.00000000');
            }).then(() => done());
        });
        it('should clone Money with the same asset but different amount (from tokens)', (done) => {
            Money.fromTokens('100', fakeFOUR).then((money) => {
                ...
                getChai_1.expect(emptyClone.toCoins()).to.equal('100000');
                getChai_1.expect(emptyClone.toTokens()).to.equal('10.0000');
            }).then(() => done());
        });
        it('should convert Money to another instance of Money with another Asset [4, 8]', (done) => {
            Money.fromTokens('100', fakeFOUR).then((money) => {
                ...
                getChai_1.expect(changedMoney.toTokens()).to.equal('400.00000000');
                const changedMoney2 = money.convertTo(fakeWAVES, 4);
                getChai_1.expect(changedMoney2.toTokens()).to.equal('400.00000000');
            }).then(() => done());
        });
        it('should convert Money to another instance of Money with another Asset [8, 4]', (done) => {
            Money.fromTokens('100', fakeWAVES).then((money) => {
                ...
                getChai_1.expect(changedMoney.toTokens()).to.equal('25.0000');
                const changedMoney2 = money.convertTo(fakeFOUR, 0.25);
                getChai_1.expect(changedMoney2.toTokens()).to.equal('25.0000');
            }).then(() => done());
        });
        it('should convert Money to another instance of Money with another Asset [8, 8]', (done) => {
            Money.fromTokens('100', fakeWAVES).then((money) => {
                ...
                getChai_1.expect(changedMoney.toTokens()).to.equal('200.00000000');
                const changedMoney2 = money.convertTo(fakeEIGHT, 2);
                getChai_1.expect(changedMoney2.toTokens()).to.equal('200.00000000');
            }).then(() => done());
        });
        it('should return the existing Money when it is being converted to Money with the same Asset', (done) => {
            Money.fromTokens('100', fakeWAVES).then((money) => {
                ...
                getChai_1.expect(changedMoney.toTokens()).to.equal('100.00000000');
                getChai_1.expect(changedMoney).to.equal(money);
                const changedMoney2 = money.convertTo(fakeWAVES, 2);
                getChai_1.expect(changedMoney2.toTokens()).to.equal('100.00000000');
                getChai_1.expect(changedMoney2).to.equal(money);
            }).then(() => done());
        });
        it('should return a proper BigNumber instance (from tokens)', (done) => {
            Money.fromTokens('1.123', fakeWAVES).then((money) => {
                ...
                getChai_1.expect(coins.isBigNumber).to.be.true;
                getChai_1.expect(coins.toFixed(0)).to.equal('112300000');
                const tokens = money.getTokens();
                getChai_1.expect(tokens.isBigNumber).to.be.true;
                getChai_1.expect(tokens.toFixed(8)).to.equal('1.12300000');
            }).then(() => done());
        });
        it('should return a proper BigNumber instance (from coins)', (done) => {
            Money.fromCoins('100000000', fakeWAVES).then((money) => {
                const coins = money.getCoins();
                getChai_1.expect(coins.isBigNumber).to.be.true;
                getChai_1.expect(coins.toFixed(0)).to.equal('100000000');
                const tokens = money.getTokens();
                getChai_1.expect(tokens.isBigNumber).to.be.true;
                getChai_1.expect(tokens.toFixed(8)).to.equal('1.00000000');
            }).then(() => done());
        });
        it('should convert to JSON', (done) => {
            Money.fromTokens('1000', fakeWAVES).then((money) => {
                getChai_1.expect(JSON.stringify(money)).to.equal('{"assetId":"WAVES","tokens":"1000.00000000"}');
            }).then(() => done());
        });
        it('should convert to a string', (done) => {
            Money.fromTokens('1000', fakeWAVES).then((money) => {
                getChai_1.expect(money.toString()).to.equal('1000.00000000 WAVES');
            }).then(() => done());
        });
    });
    describe('planned failures', () => {
        it('should throw an error when a numeric value is passed', () => {
            getChai_1.expect(() => Money.fromCoins(10, fakeWAVES)).to.throw();
            getChai_1.expect(() => Money.fromTokens(10, fakeWAVES)).to.throw();
        });
    });
});
